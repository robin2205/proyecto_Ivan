...
class ControladorClientes{
	...
	public function ctrNuevoClientedesdeInicio(){
		if(isset($_POST["nombreCliente"]) || isset($_POST["documentoCliente"])){
			if(preg_match('/^[a-zA-ZáéíóúñÁÉÍÓÚÑ ]+$/',$_POST["nombreCliente"]) && preg_match('/^[a-zA-Z0-9]+$/',$_POST["documentoCliente"]) && preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/',$_POST["emailCliente"]) && preg_match('/^[0-9-() ]+$/',$_POST["contactoCliente"]) && preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/',$_POST["direccionCliente"]) && preg_match('/^[0-9-]+$/',$_POST["fechaNacimientoCliente"])){
				# Válidamos que la persona sea mayor de Edad
				date_default_timezone_set('America/Bogota');
				$edad=intval((strtotime("now")-strtotime($_POST["fechaNacimientoCliente"]))/31536000);
				if($edad>=18){
					...
					$datos=array("nombre"=>$_POST["nombreCliente"],"documento"=>$_POST["documentoCliente"],"email"=>$_POST["emailCliente"],"contacto"=>$_POST["contactoCliente"],"direccion"=>$_POST["direccionCliente"],"fecha_nacimiento"=>$_POST["fechaNacimientoCliente"]);
					$respuesta=ModeloClientes::mdlNuevoCliente($tabla,$datos);
					if($respuesta=="ok"){
						# Mostramos una alerta suave
						echo '<script>
